...
To improve program performance, the kernel uses **in-BL range** space (starting from `0xE8414`) for the text section, which is quite different from custom modifications.

For other data and contents, we mainly use free space starting from `0xB2A604`.

In order to collaborate with FEBuilderGBA and make DEMOs based on the kernel, we also need to define some important data at these fixed locations:
...
Since the entire source code is compiled at once, CHAX can offer better free-RAM-space control. Free-RAM-space (unused memory) from vanilla that has been previously identified by other wizards can be referenced in [StanH's DOC](https://github.com/StanHash/DOC/blob/master/FREE-RAM-SPACE.md). Here we mainly use space starting at `0x02026E30` with size `0x2028`, which is the debug print buffer in vanilla (and unused).
...
Developers should ensure that used free-RAM space does not overflow, which means the assertion `(gKernelUsedFreeRamSpaceTop > FreeRamSpaceTop)` should be valid. We have also added detection for RAM space overflows; CHAX will auto-detect an overflow error on [game-init](../Kernel/Wizardry/Common/GameInitHook/source/GameInit.c#L14).
...
> [!WARNING]
> Make sure that the allocated space is 32-bit aligned.
