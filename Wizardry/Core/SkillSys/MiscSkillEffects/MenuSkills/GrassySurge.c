...
u8 GrassySurge_Usability(const struct MenuItemDef * def, int number)
{
...
static u8 GrassySurge_OnSelectTarget(ProcPtr proc, struct SelectTarget * target)
{
    ...
    gActionData.unk08 = SID_GrassySurge;
    gActionData.unitActionType = CONFIG_UNIT_ACTION_EXPA_ExecSkill;

    return TARGETSELECTION_ACTION_ENDFAST | TARGETSELECTION_ACTION_END | TARGETSELECTION_ACTION_SE_6A | TARGETSELECTION_ACTION_CLEARBGS;
}

u8 GrassySurge_OnSelected(struct MenuProc * menu, struct MenuItemProc * item)
{
    ...
    StartSubtitleHelp(
        NewTargetSelection_Specialized(&gSelectInfo_PutTrap, GrassySurge_OnSelectTarget),
