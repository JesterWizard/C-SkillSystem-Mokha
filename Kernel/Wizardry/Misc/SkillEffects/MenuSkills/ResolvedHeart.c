...
u8 ResolvedHeart_Usability(const struct MenuItemDef * def, int number)
{
    ...
    if (CheckBitUES(gActiveUnit, UES_BIT_RESOLVED_HEART_SKILL_USED))
        return MENU_NOTSHOWN;
...
u8 ResolvedHeart_OnSelected(struct MenuProc * menu, struct MenuItemProc * item)
{
    ...
    gActionData.unk08 = SID_ResolvedHeart;
...
{
    SetBitUES(gActiveUnit, UES_BIT_RESOLVED_HEART_SKILL_USED);
	return true;
}
